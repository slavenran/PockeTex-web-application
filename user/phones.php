...
<head>
    ...
    <script>
        ...
        function fetch_data() {
            $.ajax({
                type: 'post',
                url: '../api/phones/read_phones_with_manufacturers.php',
                data: {
                    id: $('#manufacturer').val(),
                    model: $('#search').val(),
                },
                success: function (response) {
                    if(response['message'] === "Nothing") {
                        ...
                        var htmlString = "<table class='table table-striped' style='width: 100%; height: 100%; color: #3d3b3c'>" +
                            ...
                            "<td style='background-color:  #d1e8e2;  text-align: center'>Model</td>" +
                            "<td style='background-color:  #d1e8e2;  text-align: center'>Year</td>" +
                            "<td style='background-color:  #d1e8e2;  text-align: center'>Rating</td></tr>";
                        for(var i = 0; i < response.length; i++) {
                            ...
                            htmlString += "<td style='text-align: center'>" + response[i].averageRat + "</td>";
                            htmlString += "</tr>";
                        }
                        htmlString += "</table>";
        ...
        function fetch_data_search() {
            // if (document.getElementById("manufacturer").value != "ori") {
                $.ajax({
                    type: 'post',
                    url: "../api/phones/read_phones_with_manufacturers.php",
                    data: {
...
<div class="pozadina  col-xs-12 col-sm-12 col-md-9 col-lg-9 col-xl-10">
    <div class="hello container">
        <form id="form" name="form">
            ...
            <br/>
            <div class="container">
            <select id="manufacturer" class="select-css" style="margin-left: 50px" onchange="fetch_data()">
                <option value="ori">Manufacturer...</option>
                <?php
            ...
            <input type="text" placeholder="Search models..." class="search" name="search" id="search"
                   onkeyup="fetch_data_search()">
            </div>
